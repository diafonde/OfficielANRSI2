              ...
              ['fr', 'ar', 'en'].forEach(lang => {
                ...
                if (langContent && 
                    (langContent.heroTitle || 
                     (langContent.values && langContent.values.length > 0))) {
                  hasData[lang] = true;
                }
              });
  ...
  populateForm(content: StrategicVisionContent): void {
    ...
    ['fr', 'ar', 'en'].forEach(lang => {
      ...
      if (langContent) {
        ...
        if (langContent.values && Array.isArray(langContent.values)) {
          langContent.values.forEach(value => {
            if (value && value.title) {
              this.addValue(value, lang);
            }
          });
        }
      }
  ...
  onSubmit(): void {
    ...
    (['fr', 'ar', 'en'] as const).forEach(lang => {
      ...
      if (langContent) {
        ...
        translations[lang] = {
          title: langContent.heroTitle || 'Vision Stratégique',
          heroTitle: langContent.heroTitle || '',
          heroSubtitle: langContent.heroSubtitle || '',
