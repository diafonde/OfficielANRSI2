              ...
              if (translation && translation.content) {
                try {
                  ...
                  if (parsedContent.heroTitle || 
                      (parsedContent.researchPriorities && parsedContent.researchPriorities.length > 0)) {
              ...
              ['fr', 'ar', 'en'].forEach(lang => {
                ...
                if (langContent && 
                    (langContent.heroTitle || 
                     (langContent.researchPriorities && langContent.researchPriorities.length > 0))) {
                  hasData[lang] = true;
                }
              });
  ...
  populateForm(content: PrioritesRecherche2026Content): void {
    ...
    ['fr', 'ar', 'en'].forEach(lang => {
      ...
      if (langContent) {
        ...
        if (langContent.introParagraphs && Array.isArray(langContent.introParagraphs)) {
          langContent.introParagraphs.forEach(paragraph => {
            if (paragraph) {
              this.addIntroParagraph(paragraph, lang);
            }
          });
        }
        if (langContent.researchPriorities && Array.isArray(langContent.researchPriorities)) {
          langContent.researchPriorities.forEach(priority => {
            if (priority && priority.title) {
              this.addResearchPriority(priority, lang);
            }
          });
        }
      }
